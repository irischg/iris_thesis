# Thesis Literature Evidence Registry v7.3
...
**v7.3 forward reference (additive; does not alter the Scripts 02-05 finding above):** The required block/month holdout validation was subsequently and separately performed as Step 17a; a corrected authority-compatible promotion-source candidate and paired Step 17c evidence were then produced. See **[P7]** for that project evidence, its exact scope, and its role boundary. The finding above -- that short-gap validation alone does not certify long-block reconstruction -- is precisely why Step 17a's independent validation was required, and remains true under Framework v7.3.
...
- `winter_pv_sensitivity_applied = True` and `winter_pv_sensitivity_method = cwa_hourly_ghi_ratio_median_all_eligible_v7_2` for all 1,463 rows;
...
**Data-boundary note (reported, not treated as a blocker):** this candidate artifact's own `pv_reconstruction_method` categorical field for the same 1,463 rows still reads the inherited label `unavailable_zero_mainline`, carried over from the v7.2 base-column semantics under which this artifact was built as a sensitivity branch; it does not describe the actual reconstruction method now populating `pv_available_kw`/`kwh` in this candidate (that method is correctly recorded in `winter_pv_sensitivity_method`). A future canonical reconstructed-mainline artifact build must correct this self-descriptive field so a single column accurately names the applied method for the 1,463-hour block; this is a provenance-labeling item for that future build, not evidence against the reconstruction itself.
...
**Design:** paired EOB and one representative Layer A case (`a0.80_b08`) solved under (i) the mainline (zero-winter) PV input and (ii) the corrected reconstructed-winter-PV sensitivity input, holding every other input, tariff, cost package, and solver setting identical (`status: PASS`; monetary basis: constant NTD-2023/year).
...
**Economic-impact mechanism (as reported by the artifact, not re-derived here):** crediting the reconstructed winter PV increased `pv_available_kwh`/`pv_used_kwh` by the 39,484.42 kWh block identified in P7-C for both the EOB and the `a0.80_b08` case, and reduced grid-import energy correspondingly; the reported resilience-premium delta between the two paired runs was small relative to the premium's own magnitude (`delta_ntd2023_per_year = -4.655894`, against `mainline_ntd2023_per_year = 41,066,852.95` and `winter_pv_pct_of_eob = 61.64%`).
...
## 17.3 Framework v7.3 to Registry v7.3 coverage matrix

| Framework location | Claim | Evidence type | Registry v7.3 record/section | Support level | Anti-claim / boundary | Status |
|---|---|---|---|---|---|---|
| Sec 4.1.3 role reversal (`PV_t^base = reconstructed` for `pre_system`/`missing_winter`) | reconstructed full-year PV is v7.3 mainline; zero-winter is sensitivity | B (project) + C (framework decision) | Claim-to-source map (Sec 12, both rows); P7 evidence-role summary | Full: numbers verified from primary artifacts (P7-C, P7-D) | not observed truth; not exact recovery | MAPPED |
...
| Sec 4.1.3 long-block validation requirement | short-gap validation cannot be extrapolated to the 1,463 h block; separate holdout validation required | A (R23/R32/R33/I5 boundary) + B (P1, P7-A) | P1-E (historical finding, unchanged) + v7.3 forward reference; P7-A (Step 17a design and verdict) | Full | literature does not itself certify the long block; P7-A is the certifying evidence | MAPPED |
...
| Sec 16.1 sensitivity role reversal | zero-winter demoted to conservative stress/sensitivity | C (framework) + B (P1 historical) | Claim-to-source map second new row; Sec 14/15 updated bullets | Full | zero-winter is not "truth" or an observed no-generation period | MAPPED |
...
All nine framework-level reconstructed-PV claims identified from a full read of the accepted Framework v7.3 primary bytes are mapped. No unmapped acceptance-relevant claim was found.
...
No Framework/Registry contradiction was found.

## 17.10 Stale-wording audit disposition

A complete, independently re-derived stale-wording inventory of this candidate document is recorded in the accompanying checkpoint (`docs/checkpoints/literature_evidence_registry_v7_3_reconstructed_pv_alignment_candidate_2026-09-23.md`), not duplicated here to avoid a second, potentially drifting count. Acceptance target: zero Category-D (stale/contradictory/ambiguous) occurrences.

## 17.11 Candidate disposition

本 Registry successor 的文件內狀態為：

> **LITERATURE / EVIDENCE REGISTRY V7.3 RECONSTRUCTED-PV MAINLINE ALIGNMENT -- CANDIDATE PASS**
