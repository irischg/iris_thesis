# 研究框架 v7.2（2026-08-24，post-audit economic integration / production-interface freeze）

> **Version lineage:** v7.2 is the integrated successor to `research_framework_v7_1_2026-08-16.md`. It preserves the v7.1 research questions, Layer A/B/DG boundaries, A1–A4 / B1–B2 methodological resolutions, preprocessing empirical freeze, equations, and claim boundaries except where explicitly superseded below. The substantive v7.2 changes are limited to post-v7.1 implementation/audit status, ex-ante PNNL cost-scale selection, monetary-basis harmonization, and tariff/cost parameter traceability.

> **Source manifest / provenance**
>
> | Source artifact | Role in v7.2 | SHA-256 |
> |---|---|---|
> | `research_framework_v7_1_2026-08-16.md` | Base authoritative framework | `5baf78dc6b2ee7c60159d095029d9a15b114e9e8385e53d6376fd7269db06624` |
> | post-v7.1 production/audit outputs through Script 13c | Project implementation / parameter / tariff evidence used to update status and freeze remaining economic decisions | repository-tracked; exact artifact hashes remain in project outputs |
> | `research_framework_v7_2_2026-08-24.md` | Integrated successor / current framework source of truth | `SELF-HASH: record externally after repository commit` |
>
> **Supersession rule:** v7.2 is the current framework source of truth. v7.1 and earlier framework/amendment files remain archived for provenance, regression, and change audit only. In particular, the v7.1 §16.1 rule that selected the PNNL 1 MW / 10 MW cost bracket from a preliminary optimized \(P^B\) range is **explicitly superseded** because it creates circular dependence between cost assumptions and the optimized BESS size.

> **Current implementation status:** **The data-integration, billing-demand calibration, PNNL cost-package construction, degradation-semantics audit, Taipower tariff-registry audit, and pure-season bill-component regression have progressed through Script 13c; Script 13c is CLOSED / PASS.** Gate 1 (cost-scale selection) and Gate 2 (monetary-basis harmonization) are methodologically CLOSED in this v7.2 specification. EOB and the final 81-point Layer A production run have not yet been executed. Remaining unchecked items in §32 are implementation, production-interface, post-solve validation, sensitivity, or final-rerun acceptance gates and do **not** reopen the closed methodology.
...
>
> **版本原則**：v7.2 直接以 v7.1（2026-08-16）為完整 base framework，採增量更新而非重寫。v7.1 的 preprocessing empirical freeze、A1–A4、B1–B2、Layer A/B/DG boundaries 與主要 equations 全部延續；本版只正式整合 v7.1 之後已完成的 implementation/audit evidence，並關閉兩個 post-audit economic integration gates。除本版明確標示 `SUPERSEDED` 的條文外，不得以 v7.2 名義重新解讀或推翻 v7.1 已鎖定的方法。
>
> **文獻分流**：本框架只記錄 framework-level decision、source role 與 claim boundary；Q1 文獻、official source、project evidence 與 exact parameter lineage 仍由 Literature/Evidence Registry 與 machine-readable parameter registry 控制。v7.2 Registry 將在本 Framework freeze 後另行對齊。
>
> **v7.2 相對於 v7.1 的核心更新**：
>
...
> 12. final EOB、81-point Layer A、post-solve exact optimized demand maxima、rainflow ex-post validation、winter-PV long-block sensitivity與後續 Layer B/DG 分析仍屬 downstream implementation / validation，不因 v7.2 framework freeze 而被標記為完成。
...
## 0.3 已定方法、implementation progress 與 remaining production gates

A1–A4、B1–B2 以及 v7.2 Gate 1–2 均 **methodologically closed**。以下狀態用來區分「已完成的 production/audit evidence」與「仍待 EOB / Layer A 前後完成的 implementation / validation」，不重新開啟方法選擇。

### 已完成：data / parameter / tariff audit chain

- **Scripts 01–05 component preprocessing CLOSED / PASSED.**
- **Script 06 final integrated annual input completed / audited**：current Load/PV baseline、observed columns、interval-start chronology、calendar / summer / TOU / billing tags已整合為 downstream canonical annual input；pre-v7 同名 legacy artifacts 不得取代 current artifact。
...
### 下一個 production-interface gate

在 EOB / Layer A 前，需把已關閉的方法決議轉成單一 machine-readable production interface：
...
- 14a production-interface preflight 必須在 EOB 前檢查 monetary basis、package role、tariff source/normalized layer、\(\kappa\)、CRF與 legacy contamination。

### 仍待 downstream implementation / validation

- post-solve exact demand maxima：EOB / Layer A solve後由 optimized calibrated-hourly grid profile重算；
- ex-post rainflow/cycle-depth validation：不嵌入 sizing objective；
- winter-PV long-block alternative：先做 block/month holdout validation，再做 economic sensitivity；
- final EOB + 81-point Layer A；
...
> 本表是 framework-level snapshot，用來防止 production code、registry 與正文出現新舊參數混用。v7.2 已將核心 BESS cost / billing calibration parameters更新為 populated / scripted-audited status；仍未完成的項目以 implementation / validation gate另行標示，不得以 legacy值補缺。
...
因此 legacy 81-point outputs 只保留作 regression targets / structural hypotheses。final EOB + 81-point Layer A 必須在 **v7.2 specification** 下重跑。
...
## Phase 1：v7.2 production-interface freeze

1. **Scripts 01–06 data preprocessing / final annual integration CLOSED / PASSED.**
2. **Billing-demand calibration scripted reproduction CLOSED**；\(\kappa\) 由 canonical observed Load–PV + billing registry重建。
3. **PNNL dual-bracket cost package construction CLOSED**：4/6/8/10 h fitting、FOM、DC Storage Block \(C_{rep}\)、2023 USD→NTD-2023（FX 31.150）、bracket-specific \(\lambda_k\) derivation完成。
4. **Xu intertemporal degradation semantics audit CLOSED**；hourly-reset legacy proxy維持排除。
5. **13b tariff registry gate CLOSED / PASS；13c pure-season bill-component regression CLOSED / PASS.**
6. **Gate 1 CLOSED**：10 MW mainline / 1 MW cost-scale sensitivity；禁止 outcome-dependent bracket switching。
7. **Gate 2 CLOSED**：constant NTD-2023 objective / real 5% discount-rate convention；raw nominal Taipower tariff保留作validation source。
8. **NEXT — Script 14a production-interface preflight**：建立/驗證 mainline package selector、1 MW sensitivity selector、constant-2023 tariff layer、monetary metadata、CRF、\(\kappa\)、legacy-contamination guards與 solver-facing parameter interface。
9. Implement/verify final solver-facing tariff cost expressions using normalized NTD-2023 rates while retaining raw nominal audit layer。
10. Implement/verify post-solve exact demand reporting + ex-post rainflow validator接口；實際 diagnostics在 EOB / Layer A solve後產生。
...
- BESS cost source：v7.2 mainline為 PNNL v2024-derived **10 MW-scale LFP planning package**；1 MW-scale package為cost sensitivity；完整 citation/parameter extraction、monetary normalization與Q1 scale-economy evidence移至registry。
...
| 資產 | 處理 |
|---|---|
| Layer A CSV | 僅作 regression input；v7.2 specification 已整合 post-audit economic decisions，final EOB + 81-point Layer A 必須依 v7.2 重跑 |
...
| 舊圖 | 不直接進 final thesis；用 v7.2 final data regeneration；site-frontier 圖保留 archive only |
...
1. **Data preprocessing / annual integration已完成**：Scripts 01–06 current artifacts為canonical source；legacy annual inputs不得回流。
...
6. **Script 14a production-interface preflight**：確認 mainline/sensitivity package routing、raw nominal vs normalized tariff分層、monetary metadata、CRF、\(\kappa\)、legacy guards。
...
9. winter-PV economic sensitivity：先完成long-block/month holdout validation，再啟用alternative reconstruction。
...
19. Chapter 4 只填 final/audited v7.2 outputs，legacy values 放在研究紀錄而非正文結論。
...
v7.2 現在即為 **authoritative coding specification**。下列項目是 **final production-results acceptance gate**：在將 EOB、Layer A/Layer B/DG outputs 視為可進入論文正文的 final/audited production results 前，逐項確認。未勾選不阻擋 coding，但 production run 不得以 legacy／pending 值補缺，且未通過相應 gate 的輸出不得標記為 final。
...
- [x] **Final integrated annual input passed (Script 06)**：
...
- [ ] final EOB + 81-point Layer A 已依 v7.2 重跑。
...
本節只記錄 v7.2 相對於 v7.1 的 post-audit economic-integration changes；它不建立新的 RQ，也不重開 A1–A4 / B1–B2。
...
## 34.5 Still pending after v7.2 framework freeze
