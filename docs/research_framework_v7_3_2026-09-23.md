# 研究框架 v7.3（2026-09-23，reconstructed-PV mainline successor candidate）

> **Version lineage:** v7.3 is the integrated, additive candidate successor to `research_framework_v7_2_2026-08-24.md`. It preserves the v7.2 research questions, Layer A/B/DG boundaries, A1–A4 / B1–B2 methodological resolutions, equations, cost/tariff/degradation rules, and claim boundaries except for the explicitly stated reconstructed-PV role reversal. The substantive v7.3 change is limited to promoting the separately validated 1,463-hour reconstructed winter PV block to the best-estimate planning baseline and reclassifying the prior zero-winter treatment as a conservative stress/sensitivity.

> **Source manifest / provenance**
>
> | Source artifact | Role in v7.3 candidate | SHA-256 |
> |---|---|---|
> | `research_framework_v7_2_2026-08-24.md` | Accepted predecessor; remains authoritative until independent acceptance of this candidate | `bfe724a35dd019b9f29456a7a7b569e132aa63b399c21c82d2c69a9fd934c8d5` |
> | `data/processed/alternatives/annual_input_v7_2_winter_pv_sensitivity_corrected_authority_project_venv_2026-09-18.parquet` | Corrected 17b reconstructed-PV promotion-source candidate; not canonical | `1c1dbc265b092415e649bba23232f645f7ba5c74e0f074f2914c1ed7ac9d7cd9` |
> | Step 17a / corrected 17b / Step 17c evidence | Validation and paired-sensitivity evidence retained with their original acceptance boundaries | repository-tracked; exact artifact hashes remain in their immutable audit records |
> | `research_framework_v7_3_2026-09-23.md` | Additive successor candidate | `SELF-HASH: recorded in the candidate checkpoint after generation` |
>
> **Supersession rule:** this v7.3 file is a candidate and is not self-accepting. v7.2 remains the current accepted framework until an independent audit accepts this successor. Once accepted, v7.3 supersedes v7.2 only where this document explicitly changes the long-block PV planning role; all historical frameworks, checkpoints, and results remain preserved for provenance, regression, and change audit. The v7.1 §16.1 cost-bracket rule remains superseded exactly as in v7.2.

> **Candidate status:** **FRAMEWORK V7.3 RECONSTRUCTED-PV MAINLINE SUCCESSOR — CANDIDATE PASS**. The reconstructed-PV mainline methodology is **DECIDED** in this candidate. Registry v7.3 is `NOT_YET_CREATED / NOT_YET_ACCEPTED`; the future canonical reconstructed-mainline annual artifact is `NOT_YET_CREATED / NOT_YET_AUTHORIZED`; canonical routing is `NOT_YET_AUTHORIZED`; Step 17c EOB adjudication remains candidate evidence only; representative cases, Steps 11A/11B/11C, and the final 81-point production rerun are not closed or authorized under v7.3.
...
>
> **版本原則**：v7.3 直接以 v7.2（2026-08-24）為完整 base framework，採 additive successor 而非改寫舊版。A1–A4、B1–B2、Gate 1、Gate 2、Layer A/B/DG boundaries、SOC 10–90%、\(\eta_c=\eta_d=0.90\)、tariff、degradation、annual regular contract-capacity（CC）semantics、alpha–beta grid、valid-start non-circular semantics、Layer-A analytical reserve formula 與主要 equations 全部延續。本版只改變 1,463-hour long-block PV 的 planning-role assignment；不得以 v7.3 名義重開其他已鎖定方法。
>
> **文獻分流**：本框架只記錄 framework-level decision、source role 與 claim boundary；Q1 文獻、official source、project evidence 與 exact parameter lineage 仍由 Literature/Evidence Registry 與 machine-readable parameter registry 控制。Registry v7.3 尚未建立或接受；在完成獨立對齊與接受前，不得把 Registry v7.2 改稱 v7.3，也不得宣稱 framework–registry pair 已完成 promotion。
>
> **v7.3 candidate 相對於 v7.2 的唯一方法角色更新**：
>
> 1. **Reconstructed-PV mainline methodology DECIDED**：2024/11–12 的 1,463-hour corrected reconstructed PV block 進入 full-year best-estimate planning baseline；先前 zero-winter treatment 改列 conservative stress/sensitivity。
> 2. **Required epistemic statement:** The winter PV reconstruction is a model-based, weather-informed, separately validated estimate for planning use; it is not observed meter data, not exact ground truth, and not an exact recovery of the missing historical series.
>
>    **Required thesis-language boundary:** The prolonged unavailable winter PV block is represented using a separately validated weather-informed reconstruction as the best-estimate planning baseline. The prior zero-availability treatment is retained as a conservative stress/sensitivity case. Reconstructed values are model-based estimates rather than observed historical PV measurements.
> 3. 同一套 reconstructed full-year \(PV_t^{base}\) 必須一致送入 EOB/economic optimization、Layer A 的 \(R_t\)、\(P_t^{out}\)、binding classification 與 outage replay，以及 baseline Layer B。禁止 EOB 採 reconstructed PV、但 Layer A/B 又用 zero-winter PV 的 hybrid/no-credit routing。
> 4. corrected 17b promotion-source candidate 為 `data/processed/alternatives/annual_input_v7_2_winter_pv_sensitivity_corrected_authority_project_venv_2026-09-18.parquet`（SHA-256 `1c1dbc265b092415e649bba23232f645f7ba5c74e0f074f2914c1ed7ac9d7cd9`）；它尚未 canonical。未來 canonical artifact 的 path/hash 為 `NOT_YET_CREATED / NOT_YET_AUTHORIZED`，不得預先杜撰。
> 5. historical billing calibration 繼續使用 observed Load/PV；\(\kappa\) 的定義、估計樣本與數值不因 reconstructed baseline 改變。
> 6. Layer A 公式不變，包含 consistency replay 中 outage-period PV surplus 不得 recharge 的既有規則；本次是 input-role correction，不是 equation change。
>
...
A1–A4、B1–B2 以及 v7.2 Gate 1–2 均 **methodologically closed**。v7.3 reconstructed-PV mainline role assignment 為本 candidate 的 **DECIDED methodology**，但 framework 本身仍待 independent acceptance。以下狀態區分歷史 evidence、candidate evidence 與尚未授權的 v7.3 production work；不重新開啟已關閉的方法。

### 已完成：data / parameter / tariff audit chain

- **Scripts 01–05 component preprocessing CLOSED / PASSED.**
- **Script 06 v7.2 integrated annual input completed / audited**：其 Load/PV baseline、observed columns、interval-start chronology、calendar / summer / TOU / billing tags構成 accepted v7.2 historical/canonical lineage；pre-v7 同名 legacy artifacts 不得取代它。此 artifact 不因 v7.3 candidate 而被靜默改寫。
...
### 下一個 v7.3 governance / production-interface gates

在任何 v7.3 EOB / Layer A 執行前，必須依序完成：

- independent audit 接受本 framework candidate；
- 建立並接受 Registry v7.3，使 framework–registry pair 同版對齊；
- 由 corrected promotion-source candidate 建立新的 canonical reconstructed-mainline annual artifact，並記錄新 path/hash；目前為 `NOT_YET_CREATED / NOT_YET_AUTHORIZED`；
- 明確授權 production routing，使 EOB/economic、Layer A 與 baseline Layer B 共同讀取該同一 artifact；
- mainline run 明確載入 10 MW package；sensitivity run明確載入完整 1 MW package；禁止依 solver result動態切換；
- 將 official case-year nominal Taipower tariff保留為 validation layer，另依正式 price-index/deflator provenance產生 constant NTD-2023 optimization layer；
- parameter registry同時保存 raw source、normalized value、currency/base year、annualization status、mainline/sensitivity role；
- production-interface preflight 必須在 EOB 前檢查 monetary basis、package role、tariff source/normalized layer、\(\kappa\)、CRF、PV artifact identity、same-artifact routing 與 legacy contamination。

### 仍待 downstream implementation / validation

- Step 17a `PASS_FOR_SENSITIVITY`、17b sensitivity-only、corrected Sep-18 source/run與17c paired-sensitivity/adjudication evidence均保留原角色；17c EOB adjudication仍是 candidate evidence，不能代替 v7.3 canonical rerun；
- representative cases 與 Steps 11A/11B/11C 尚未依 v7.3 關閉；
- post-solve exact demand maxima：v7.3 EOB / Layer A solve後由 optimized calibrated-hourly grid profile重算；
- ex-post rainflow/cycle-depth validation：不嵌入 sizing objective；
- final EOB + 81-point Layer A 尚未在 v7.3 下授權；
- Layer A 後才 freeze Layer B benchmark subset；
- DG CAPEX/FOM/fuel/emission parameter audit與 external coverage extension可在主線 Layer A 後平行完成。
...
### 4.1.3 2024/11–12 prolonged PV unavailability — v7.3 role reversal
...
v7.3 candidate 的 best-estimate planning baseline固定採用經 corrected 17b source保存、並由 Step 17a long-block/monthly holdout與 Step 17c paired comparison分開驗證的 reconstructed PV：
...
#### Promotion source與 canonical boundary
...
- source SHA-256：`1c1dbc265b092415e649bba23232f645f7ba5c74e0f074f2914c1ed7ac9d7cd9`；
- current role：validated/corrected **promotion-source candidate**, not canonical；
- future canonical reconstructed-mainline artifact：`NOT_YET_CREATED / NOT_YET_AUTHORIZED`；
- 不得把上列 corrected source path改稱 future canonical path，也不得先填入不存在的 future hash。

#### Mainline / sensitivity routing

- reconstructed full-year PV = EOB/economic、Layer A adequacy與 baseline Layer B 共用的 mainline best-estimate input；
- prior zero-winter definition = conservative stress/sensitivity，描述為 **conservatively assigned zero PV availability due to unavailable observations**，不得寫成「實際 PV generation = 0」；
- 禁止 hybrid/no-credit rule：不得讓 EOB/economic接受 reconstructed winter PV credit，卻讓 Layer A 的 \(R_t\)、\(P_t^{out}\)、binding classification、outage replay或 baseline Layer B 回退至 zero-winter PV；
- zero-winter branch若執行，必須作為明確、成套、可追溯的 conservative stress/sensitivity；不得與 reconstructed-mainline consumer混接；
- historical billing calibration與 \(\kappa\) reproduction不切換到 reconstructed values，仍使用 observed Load/PV。
...
- original `pre_system` / `missing_winter` status、`pv_reconstructed` flag、planning-role（mainline reconstructed或zero-winter stress）與 canonical/candidate status。
...
baseline.*              ↓
      ...
      promotion-source candidate
                ↓
      v7.3 canonical reconstructed-mainline input
      [NOT_YET_CREATED/AUTHORIZED]
```
...
先前 zero-winter annual definition不是 \(\gamma_{PV}=1.0\) baseline，而是獨立的 conservative stress/sensitivity。Layer B baseline不得用 zero-winter annual series取代 reconstructed-mainline series；這項禁止不影響 Layer B event-level \(\gamma_{PV}\) availability stress design。
...
- **winter-PV treatment（v7.3 role reversal）**：reconstructed full-year PV為 mainline best estimate；prior zero-winter annual treatment為 conservative stress/sensitivity。比較必須成套涵蓋 EOB/economic、Layer A \(R\)、\(P^{out}\)、binding/outage replay及 baseline Layer B，不得只讓 economic objective取得 reconstructed credit。
...
因此 legacy與v7.2/Step-17 81-point outputs只保留其原始 regression、historical、diagnostic或candidate evidence角色。future final EOB + 81-point Layer A 必須在 **independently accepted v7.3 framework + Registry v7.3 + authorized canonical reconstructed-mainline input** 下重跑。
...
7. 2024/11–12 prolonged PV observations不可用；mainline採 model-based、weather-informed、separately validated reconstruction，因此 annual economics與deterministic historical-start adequacy部分依賴 reconstruction validity，且仍有 model/form uncertainty。它不是 observed meter data、exact ground truth、exact recovery或 probabilistic PV forecast model；prior zero-winter annual treatment保留作 conservative stress/sensitivity。adequacy結論仍是 model-conditional，不能證明 future-outage reliability或 physical islanding capability。
...
# 25. v7.3 candidate 後續執行順序

## Phase 1：governance acceptance與 canonical input promotion

1. independent audit本 additive v7.3 framework candidate；candidate不得 self-promote。
2. audit acceptance後建立並獨立接受 Registry v7.3；Registry v7.2保持 immutable accepted predecessor。
3. 以 corrected Sep-18 promotion-source candidate建立新的 canonical reconstructed-mainline annual artifact；在實際建立與授權前，其 path/hash維持 `NOT_YET_CREATED / NOT_YET_AUTHORIZED`。
4. audit新的 canonical artifact，並授權 same-artifact routing至 EOB/economic、Layer A與 baseline Layer B。
5. historical billing calibration維持 observed Load/PV，reproduce並確認 \(\kappa\) 未因 baseline promotion改變。
6. 保留且驗證既有 Gate 1/2、tariff、degradation、SOC、efficiency、annual regular CC、alpha–beta、non-circular starts與reserve-formula guards。
7. 不在本 framework-only step執行 solve、representative cases、Step 11A/11B/11C或 final 81-point production run。

## Phase 2：v7.3 Layer A production（須另行授權）
...
> 本研究不是預測停電機率，也不是替校方選擇唯一服務水準或提供 site-specific BESS 工程設計。研究先將 NTUST 原始時序整理成固定 2024-11-01 至 2025-10-31 case-year chronology，保留 observed 與 planning-baseline series；歷史帳單校準只使用 observed Load–PV，並由實際 usage periods 的四時段 billed maxima 建立 site-specific hourly-to-15-min demand proxy \(\kappa\)。年度 planning 的 best-estimate baseline使用含 1,463-hour corrected winter reconstruction的同一套 full-year PV；該 reconstruction是 model-based、weather-informed、separately validated planning estimate，不是 observation、exact truth或 exact recovery，prior zero-winter treatment則作 conservative stress/sensitivity。對每組服務比例 \(\alpha\) 與設計停電時長 \(\beta\) 掃描所有 valid historical starts，建立 conservative energy reserve \(R\) 與瞬時 power requirement。正常運轉 optimization 使用同一套 LFP BESS、10–90% SOC、固定 \(\eta_c=\eta_d=0.90\)、SOCmin 以上 constant worst-case reserve、annual regular contract capacity、**PNNL 10 MW-scale mainline cost package（1 MW sensitivity）與 constant NTD-2023 / real-5% accounting basis**，以及 PNNL-calibrated adaptation of Xu et al.'s intertemporal PWL DOD-sensitive cycle-aging formulation。停電 adequacy 不在 annual model內用同一 reserve floor硬撐，而以獨立 replay 從最低 preparedness state開始、允許消耗 reserve至 technical SOCmin；Layer A consistency replay使用同一 reconstructed PV但不利用 outage PV surplus recharge，Layer B capability stress test則可允許。billing maxima與 DOD/rainflow diagnostics都在 solve 後由實際 dispatch重算。完成 dense Layer A response surface與 targeted robustness後才選 benchmark designs做 fixed-design Layer B stress test；existing emergency DG不進主線，hypothetical campus-serving DG另以外生 coverage sweep呈現年度固定成本與單次事件燃料/碳排 trade-off。
...
| 資產 | 處理 |
|---|---|
| Layer A CSV | 僅作 regression / historical evidence；既有 v7.2/Step-17 outputs不得改標為 v7.3 final，v7.3 EOB + 81-point Layer A須在 future canonical reconstructed-mainline input與routing另行授權後重跑 |
...
| 舊圖 | 不直接進 final thesis；未來以 accepted v7.3 canonical data regeneration；site-frontier 圖保留 archive only |
...
1. **v7.2 data preprocessing / annual integration已完成**：Scripts 01–06 artifacts保留為 accepted predecessor lineage；v7.3 reconstructed-mainline canonical artifact尚未建立或授權。
2. **Billing-demand calibration已 scripted reproduction**：production \(\kappa\) 由 canonical observed grid import + usage-period billed maxima控制。
3. **PNNL cost/degradation package construction已完成**：10 MW / 1 MW full packages、4/6/8/10 h fitting、\(C_{rep}\)、\(\lambda_k\)、FX/base-year provenance已建立。
4. **Taipower institutional/bill audit已通過13b/13c**：non-duplication、2×/3×、period-specific rate與pure-season core bill components已 regression checked；May/October 50/50只保留case-validated empirical status。
5. **Gate 1 / Gate 2 framework freeze已完成**：10 MW mainline、1 MW sensitivity；constant NTD-2023 objective、real 5%。
6. **v7.3 governance gates**：independent framework acceptance、Registry v7.3建立/接受、canonical reconstructed-mainline artifact建立/稽核、same-artifact routing authorization。
7. valid-start outage indexing、annual/replay state semantics、10–90% SOC、\(\eta_c=\eta_d=0.90\)、Xu intertemporal segment-state implementation與 solver-facing unit tests通過。
8. post-solve exact demand reporting與ex-post rainflow validator可呼叫；實際結果於EOB/Layer A後產生。
9. winter-PV role：17a long-block/monthly holdout、17b sensitivity-only、corrected Sep-18 run與17c paired comparison均保留為歷史/candidate evidence；未來以 reconstructed PV作 mainline、zero-winter作 conservative stress。
10. DG cost/fuel/emission parameters可在主線 Layer A後平行完成；BESS site data不再是 blocker。
...
19. Chapter 4 只填 future final/audited v7.3 outputs；v7.2、Step-17與其他 legacy/candidate values放在研究紀錄或清楚標示其證據角色，不得冒充 v7.3 final。
...
v7.3目前是 **candidate framework，尚非 authoritative coding specification**；v7.2在 independent acceptance 前仍為 accepted predecessor。下列項目是 future v7.3 **final production-results acceptance gate**：未完成 framework/Registry/canonical-artifact/routing authorization前，不得啟動或標記 v7.3 production run；未通過相應 gate 的輸出不得標記為 final。
...
# 35. v7.3 reconstructed-PV mainline successor freeze

本節是 v7.3 相對於 v7.2 的 additive methodology delta與 current-state freeze。它只改變 prolonged-winter PV 的 planning role，不改變公式、tariff、degradation、cost package或任何已關閉的 A1–A4 / B1–B2 / Gate 1–2 decision。

## 35.1 Decision and epistemic boundary

**Methodology decision:** reconstructed full-year PV（包含 2024/11–12 共 1,463 個 unavailable-observation hours）是 v7.3 的 mainline best-estimate planning baseline；v7.2 的 zero-winter treatment改列 conservative stress/sensitivity。

下句為 mandatory thesis wording，不得弱化或改寫成 observed/true/recovered：

> **The winter PV reconstruction is a model-based, weather-informed, separately validated estimate for planning use; it is not observed meter data, not exact ground truth, and not an exact recovery of the missing historical series.**

正式 thesis-language boundary：
...
任何「EOB/economic使用 reconstructed winter PV，但 Layer A或 baseline Layer B對相同 1,463 hours設零」的 hybrid/no-credit implementation均違反本框架。zero-winter若執行，必須是完整、明示且分離的 conservative stress/sensitivity branch。
...
historical billing calibration只使用 observed Load/PV與實際 billing records。v7.3不得以 reconstructed winter PV重新估計、替換或調整 \(\kappa\)。planning-baseline promotion與billing calibration是兩個分離的 data views。
...
## 35.8 Candidate disposition

本 framework successor的文件內狀態為：

> **FRAMEWORK V7.3 RECONSTRUCTED-PV MAINLINE SUCCESSOR — CANDIDATE PASS**

這個 disposition只表示文件已完整表達決定與邊界；它不等於 canonical acceptance、Registry acceptance、artifact promotion、routing authorization或 production-run authorization。
