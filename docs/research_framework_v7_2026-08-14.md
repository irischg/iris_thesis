# 研究框架 v7（2026-08-14，version renumbering / traceability alignment）

> **Version lineage:** v7 = former v6.2.3.1. Renamed on 2026-08-14 for version simplicity; no methodological, equation, parameter, or blocker-status change accompanies this renumbering.

> **Current implementation status:** **Ready to code, not ready for final production results.** Framework v7 together with Literature/Evidence Registry v7 is the authoritative current coding specification. Unchecked items in §32 are implementation, parameter-population, reproducibility, validation, and final-rerun acceptance gates; they do **not** reopen A1–A4 or B1–B2. Earlier framework/registry versions must not be used to infer, supplement, or override current mainline methodology or unresolved production values. They may be consulted only where v7 explicitly labels them as legacy, replication, regression, provenance, historical lineage, or comparison material. If v7 marks a required value as `PENDING POPULATION` or `PENDING SCRIPTED REPRODUCTION`, production code must leave it unresolved until the v7-designated source/pipeline supplies it; it must not backfill the value from an older version.
...
>
> **版本原則**：本版沿用 v6.2.3 的全部研究問題、Layer A/B/DG 邊界、資料治理與 A1–A4 / B1–B2 方法決議；本次僅收緊 Xu et al. (2018) degradation formulation 的 attribution、citation boundary、efficiency convention 與 equation-level mapping，不重新開啟任何方法 blocker。 A1–A4、B1–B2 均視為 **methodologically closed**；尚未完成的工作只屬 implementation、parameter population、reproducibility 與 final rerun，不重新打開方法方向。主要實質更新包括：效率固定為 \(\eta_c=\eta_d=0.90\)；annual reserve floor 與 outage replay 分離；NTUST tariff/CC 與 billing-period semantics 鎖定；floating monthly demand proxy 改為 cost-facing epigraph/exceedance + ex-post exact maxima；degradation 主線鎖定為 **PNNL-calibrated adaptation of Xu et al.'s intertemporal convex PWL DOD-sensitive cycle-aging formulation**。B2 的 annualized CAPEX + FOM + operating-cost accounting 結構維持不變。Harry-era 數值與 hourly-reset degradation proxy 僅保留 legacy/replication lineage，不控制 production results。
>
...
A1–A4、B1–B2 均 **methodologically closed**。下列事項尚未完成，但屬 production implementation / reproducibility，不是重新打開 blocker：

- final cleaned/canonical dataset 依 §4.1 的 timestamp、baseline reconstruction 與 provenance 規則落地；production model input 優先使用 `annual_input_existing_pv.csv` 的 canonical schema。
...
- 建立 `calibrate_kappa.py`：從 `annual_input_existing_pv.csv` 的 `observed_load_kw - observed_pv_kw` 與 billing registry 重建 2025/01–10 十組 \(H_m,B_m\)，輸出 ratios、residuals 與 through-origin \(\kappa\)；不得只 hard-code 1.01037。
...
- 跑一次 winter-PV conservative-zero vs alternative reconstructed-PV sensitivity；site feasibility data 不列 production blocker。
...
### 4.1.2 歷史 outage-contaminated short gaps

歷史 grid outage 造成 Load 與 PV 同時為零的時段，不得直接解讀為 baseline demand/resource 同時為零。Load 與 PV 必須聯合重建，且 original observations 不覆寫。

Mainline reconstruction rules：

- **Load**：same weekday、\(\pm8\) weeks、top \(K=3\) context-matched donor blocks。
- **PV**：\(\pm30\) days、no weekday restriction、top \(K=5\) daylight/context-matched donors。
- donor candidates 排除 outage/fault/reconstructed/anomalous periods 與 contamination/recovery envelope。
- 若取得正式 outage start/restoration logs，正式紀錄優先於 inferred envelope。

保留 pseudo-gap validation 作 preprocessing robustness check；至少比較 MAE、RMSE、signed energy bias 與 event/day-level absolute energy bias。

目前 core-only 與 expanded-envelope reconstruction 對 81 個 \(R(\alpha,\beta)\) 與 \(P^{out}(\alpha)\) 均無影響；這是 robustness finding，不代表 data cleaning 可省略。

### 4.1.3 2024/11–12 prolonged PV unavailability

長時間 PV 資料不可用與短期 outage contamination 分開處理。

對正式標記為 `pre_system` 或 `missing_winter` 的 interval，mainline 採：

\[
PV_t^{base}=0
\]

並明確描述為 **conservatively assigned zero PV availability due to unavailable observations**，不得稱實際 PV generation 為零。

alternative reconstructed/synthetic winter PV 只作一次 economic sensitivity，用來檢查 EOB、annual dispatch、CC、annual cost 與 resilience premium；不作 Layer A adequacy mainline 的替代來源。

### 4.1.4 最低 provenance fields

final thesis dataset 至少保留：
...
- `pv_status`；
- `data_status`；
- regenerated `month`、`weekday`、`hour`、`TOU_tag`。

raw input files immutable；任何 reconstructed value 都必須有 provenance flag。
...
historical calibration 使用 **observed historical grid import**，不得使用 outage-reconstructed planning baseline：
...
production canonical input 使用 `annual_input_existing_pv.csv` 的：

- `observed_load_kw`；
- `observed_pv_kw`。

不得用 `baseline_load_kw - pv_available_kw` 校準歷史帳單，因為後者包含 counterfactual reconstruction，與帳單實際量測邊界不一致。
...
Mainline 以 2025/01–2025/10 十個 `solar_status=valid` usage months 做 through-origin least-squares calibration：
...
- **outage-load / short-gap reconstruction**：沿用已完成 robustness evidence；final data pipeline未變更時不重做 full sweep。
...
- pseudo-gap validation；
- core-only vs expanded-envelope reconstruction robustness；
- 81-case \(R/P^{out}\) rerun comparison。
...
## Phase 1：v7 production implementation freeze

1. Final canonical case-year dataset：timestamp-start、observed/base series、provenance flags。
2. 建立 `billing_demand_registry.csv`：usage-period indexing、regular/supplementary CC、four TOU maxima、source filenames。
3. 建立 `calibrate_kappa.py`，以 observed Load–PV 重建 2025/01–10 十組 calibration pairs並重現/更新 \(\kappa\)。
4. Implement valid-start \(\mathcal S_\beta\)，移除 outage-window circular wrap。
5. Implement \(e_t\) state、10–90% SOC、constant reserve above SOCmin、\(\eta_c=\eta_d=0.90\)。
6. Populate PNNL v2024-derived \(C_E,C_P,FOM,C_{rep}\) 與 currency/base-year metadata；derive CRF與 PWL \(\lambda_k\)。
7. Implement Xu-derived intertemporal degradation segment states、segment cyclic boundary、battery-side degradation energy accounting；禁止 hourly-reset proxy。
8. Implement case-year Taipower tariff module：5/16–10/15 summer calendar、annual regular CC、supplementary CC=0、four TOU period rules、non-duplication、2×/3× tiers；精確重現 September 2025 bill regression case。
9. Implement post-solve exact \(D_{m,q}^{exact}\)、\(D_m^{exact}\) reporting，移除 floating monthly proxy作 diagnostics。
10. 建立 automated data/billing/BESS/degradation sanity-check report與 ex-post rainflow validator。
11. 跑一次 winter-PV economic sensitivity；site feasibility data不列 production blocker。
...
| 資產 | 處理 |
|---|---|
| Layer A CSV | 僅作 regression input；v7 specification 已改變，final EOB + 81-point Layer A 必須重跑 |
...
| 舊圖 | 不直接進 final thesis；用 v7 final data regeneration；site-frontier 圖保留 archive only |
...
2. final canonical case-year dataset：timestamp-start、observed/base split、outage-gap reconstruction、provenance flags。
...
19. Chapter 4 只填 final/audited v7 outputs，legacy values 放在研究紀錄而非正文結論。
...
v7 現在即為 **authoritative coding specification**。下列項目是 **final production-results acceptance gate**：在將 EOB、Layer A/Layer B/DG outputs 視為可進入論文正文的 final/audited production results 前，逐項確認。未勾選不阻擋 coding，但 production run 不得以 legacy／pending 值補缺，且未通過相應 gate 的輸出不得標記為 final。
...
- [ ] case year exactly 2024/11/01 00:00–2025/10/31 23:00，8760 intervals。
- [ ] hour-ending → interval-start conversion 完成。
- [ ] calendar/TOU labels由 interval-start 重建。
- [ ] observed/base Load/PV 分離且 provenance flags 完整。
- [ ] outage-contaminated short gaps 已依 frozen donor rules處理。
- [ ] prolonged winter PV unavailability 依 mainline conservative-zero rule標記。
- [ ] \(\mathcal S_\beta\) 不做 circular wrap。
- [ ] core vs expanded contamination robustness 與 winter-PV economic sensitivity 已保存。
...
- [ ] final EOB + 81-point Layer A 已依 v7 重跑。
