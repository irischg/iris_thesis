# Thesis Literature Evidence Registry v7.1
...
**Compiled:** 2026-08-04  
**Updated:** 2026-08-16 — Registry v7.1 aligned with Research Framework v7.1; Scripts 01–05 preprocessing evidence integrated; PV short-gap evidence expanded to include weather/irradiance-to-PV model-selection literature and CWA institutional provenance. A1–A4 and B1–B2 remain methodologically closed.

**Version lineage:** Registry v7.1 is the integrated successor to `thesis_literature_evidence_registry_v7_2026-08-14.md`. It preserves the v7 evidence base unless explicitly revised below, and aligns preprocessing claims with `research_framework_v7_1_2026-08-16.md`. The principal substantive change is evidence-role alignment after NTUST Scripts 02–05: Load production reconstruction is now site-validated `same weekday ±6 weeks / K=1 / same-day outside-gap RMSE`; PV donor `±30 days / K=5 mean` is retained as a validated benchmark, while short-gap production reconstruction is now CWA `hourly_ghi_ratio_median` selected by head-to-head pseudo-gap validation.

> **Source manifest / provenance**
>
> | Source artifact | Role in v7.1 | SHA-256 |
> |---|---|---|
> | `thesis_literature_evidence_registry_v7_2026-08-14.md` | Base evidence registry | `d13c3674ca35c06775bc75d04acf6270db739e3baa0974e6956ae648456004d5` |
> | `research_framework_v7_1_2026-08-16.md` | Current framework alignment target | `5baf78dc6b2ee7c60159d095029d9a15b114e9e8385e53d6376fd7269db06624` |
> | Scripts 01–05 outputs in `results/data_audit/` | Project empirical preprocessing evidence | tracked by repository files / future commit hashes |
> | `thesis_literature_evidence_registry_v7_1_2026-08-16.md` | Integrated successor / current registry source of truth | `SELF-HASH: record externally after repository commit` |
>
> **Supersession rule:** From v7.1 onward, Registry v7 is retained for provenance/lineage only. Framework v7.1 + Registry v7.1 control current implementation and claim mapping. Exact site-selected preprocessing rules are controlled by project evidence (P1), not by literature precedent.
...
**Coding source-of-truth rule:** Framework v7.1 + Registry v7.1 are the only current mainline specification/evidence sources for implementation. Earlier framework/registry versions may be used only when v7.1 explicitly classifies them as legacy, replication, regression, provenance, historical lineage, or comparison material. A `PENDING POPULATION` or `PENDING SCRIPTED REPRODUCTION` value must not be filled from an older version for production use; the v7.1-designated source or reproduction pipeline controls the value.
...
## 0.1 v7.1 literature/evidence-conflict resolution summary

| 看似衝突的證據 | v7.1 處理 | 最終角色 |
|---|---|---|
...
| annual cyclic BESS state vs outage year-end wrap | 前者是年度 optimization boundary；後者會人工拼接不同 case-year 邊界，v7.1 已禁止 | R11 + P4 |
...
| Harry-era 813.75/694.4/0.07、0.55、0.532/1.596/2.128 vs current assumptions | 舊數值僅 replication/lineage；不得控制 v7.1 mainline | T2–T4 + L1 |
...
- **不能作 v7.1 PWL degradation formulation 的方法依據。** PWL mainline 的 Xu 文獻是 **Xu et al. (2018), R20**。
...
### 本研究怎麼用
R20 是 v7.1 **B1 mainline optimization formulation 的主要 methodological source**。正式名稱統一為：
...
### 不能支持
- 「±30 days, no weekday restriction, K=5 mean」是 production requirement；它在 v7.1 只保留為 validated donor benchmark；
...
### 本研究怎麼用
作為 v7.1 \(r=5\%\)、\(n=20\) yr 的近期、場域相近 literature precedent。
...
### 本研究怎麼用
作為 v7.1 **hourly temporal-resolution limitation 的主要近期文獻**。本研究因此把 hourly model 定位為 annual planning approximation，而不是 exact sub-hourly power/reliability reproduction。
...
### 本研究怎麼用
作為 R28 的 **recent corroborating evidence**，強化 v7.1 對 battery operation / SOC / power-side outputs 的 limitation wording。
...
Therefore the v7.1 fixed technical calibration set is:
...
DOD and segment widths are battery-side energy fractions. Because v7.1 defines \(p_t^{dis}\) on the AC/PCS side, degradation accounting must convert discharged energy to battery-side basis using \(p_t^{dis}\Delta t/\eta_d\) (or an algebraically equivalent segment-state formulation).

Unit convention: \(\lambda_k\) is **NTD per battery-side discharged kWh**. This is algebraically equivalent to Xu et al. Eq. (5)–(7), where \(1/\eta^{dis}\) is embedded in the AC-side marginal coefficient \(c_j\); v7.1 instead keeps \(\lambda_k\) on a battery-side basis and applies \(1/\eta_d\) to AC-side discharged energy in the objective. Do not apply both conventions simultaneously.

### 不能支持
- 直接把任一組 \(\lambda_k\) 稱為 PNNL published coefficients；
- 把 Harry-era \(C_{rep}=5107.57\) TWD/kWh 或 \((0.532,1.596,2.128)\) 鎖成 v7.1 mainline；
...
### 本研究怎麼用
支持：hourly optimization **不能聲稱精確重現 15-min billing maximum**；因此 v7.1 使用 site-specific calibrated hourly billing-demand proxy。
...
### 本研究怎麼用
支持 v7.1 calendar tagging 必須依日期 boundary，而非以整月 shortcut 標記 summer/non-summer。
...
### NTUST case setting
v7.1 optimizes **one annual regular \(CC\)**. Supplementary half/Sat-half/off-peak CCs are not optimization decisions and are fixed to the NTUST case setting recorded in the billing registry.
...
- v7.1 project pipeline uses CWA station data (project-selected station ID 466920) and retains station / source-column provenance。
- GHI / temperature inputs are aligned to the v7.1 interval-start case-year before any PV mapping is fit。
...
The former framework candidate \(\pm8\) weeks / \(K=3\) performs materially worse on the same validation design and is **not** a production rule in v7.1.
...
- CWA observations are normalized to the v7.1 interval-start 8,760-hour chronology before comparison。
...
Therefore v7.1 short-gap PV production reconstruction uses **CWA `hourly_ghi_ratio_median`**. The donor method remains a comparison benchmark.
...
The older claim that “core-only vs expanded contamination-envelope reconstruction has already been shown to have no effect on all 81 \(R/P^{out}\) cases” is **not carried forward as a current v7.1 validated finding** under the newly selected production rules.
...
Production calibration uses the **observed columns of the final v7.1 integrated annual input**:
...
The **file name is not the source of truth**. The final integrated artifact must be regenerated from current v7.1 `load_annual_baseline.*` + `pv_annual_baseline.*`, preserve Script-01 observed columns unchanged, use the exact interval-start 8,760-hour chronology, regenerate calendar/TOU/billing tags, and pass the final integration audit.

Any pre-v7 `annual_input_existing_pv.*` file is a **legacy artifact** until regenerated by the current pipeline. If that filename is reused for the final integrated artifact, provenance must show that it was rebuilt from v7.1 inputs.
...
In v7.1 this number is **not a production hard-code**. It is a regression target pending scripted reproduction from the canonical hourly file + billing registry.
...
v7.1 production `parameter_registry` 至少使用以下 12 個 machine-readable fields：
...
**Evidence role:** production reproducibility and legacy-contamination control; aligns registry with framework v7.1.1.
...
# 12. Claim-to-source map（v7.1 aligned）
...
12. **Weber (2021) + Jeong (2021) — R21–R22；Demirhan & Renwick (2018) — R23；Ramadhan et al. (2021) + Mayer & Gróf (2021) — R32–R33；CWA — I5；P1**：Load donor principles、solar-gap validation、weather→PV mapping、multi-metric method selection，以及 NTUST exact empirical preprocessing freeze。
...
# 15. 官方／場域／parameter evidence 狀態（v7.1）
...
- **final integrated annual input**：尚未完成。P2 calibration source-of-truth改為final v7.1 integrated input的observed columns；pre-v7 `annual_input_existing_pv.*`不得因檔名直接沿用。
...
- When T4 is cited, state whether the thesis is using **Table 4.2 cycles-to-end-of-life** or another PNNL endpoint. The current v7.1 calibration uses Table 4.2.
...
- Do not restore Load ±8/K3 or PV donor ±30/K5 as production rules merely because older Registry v7 recorded them; P1 v7.1 empirical evidence supersedes those roles.
...
- P2 must follow the final v7.1 integrated annual input's observed columns; an old `annual_input_existing_pv.*` is legacy until regenerated and audited.
