# 研究框架 v7.4（2026-09-26，zero-winter validation-role / observed-vs-planning PV routing / \(\kappa\) claim-boundary successor candidate）

> **Version lineage:** v7.4 is the integrated, additive **candidate** successor to the accepted, immutable `research_framework_v7_3_2026-09-23.md` (accepted SHA-256 `44e313e71ea2a01e213454b4d838a86ec674fcda4f95a3b194ed68a92115ef2a`). v7.3 remains **immutable historical provenance** and remains the current accepted methodology authority until this candidate passes an independent audit. v7.4 inherits the full v7.3 framework — research questions, research positioning, Layer A/B/DG boundaries, A1–A4 / B1–B2 / Gate 1 / Gate 2 resolutions, every equation, the \(9\times9=81\) case universe, SOC / efficiency / reserve / degradation / tariff / cost-package / monetary-basis / CRF settings, the \(\kappa\) estimator and its production value, and all claim boundaries — unchanged. Exactly **three** methodology/claim-boundary categories are changed: **(A)** the current role of the zero-winter treatment becomes historical conservative validation / provenance evidence rather than a required downstream sensitivity or stress; **(B)** the observed-versus-planning PV epistemic routing for the 1,463-hour reconstructed winter block, and for the planning layer generally, is made formal and internally consistent; **(C)** the \(\kappa\) historical-calibration versus planning-use claim boundary is made explicit without any recalibration. No equation, parameter, numerical setting, solver setting, production input, or accepted numerical result is changed by this successor.

> **Source manifest / provenance**
>
> | Source artifact | Role in v7.4 candidate | SHA-256 |
> |---|---|---|
> | `docs/research_framework_v7_3_2026-09-23.md` | **Direct predecessor**; accepted current methodology authority; immutable historical provenance; remains authoritative until independent acceptance of this candidate | `44e313e71ea2a01e213454b4d838a86ec674fcda4f95a3b194ed68a92115ef2a` |
...
> | `docs/research_framework_v7_4_2026-09-26.md` | This additive successor **candidate** | `SELF-HASH: recorded externally in the v7.4 candidate checkpoint and manifest after generation` |
>
> **Supersession rule:** this v7.4 file is a **candidate** and is **not self-accepting**. The authoring session has no authority to promote it. Framework v7.3 remains the current accepted methodology authority, and Registry v7.3 R3 remains the current accepted evidence authority, until an independent read-only Framework audit accepts this successor. Once and only if accepted, v7.4 supersedes v7.3 **solely** in the three change categories declared above and enumerated in §36; everywhere else v7.3's inherited text governs unchanged. All predecessor frameworks, registries, checkpoints, audits, freezes, manifests, and results remain preserved byte-identically for provenance, regression, historical-lineage, replication, and comparison. The v7.1 §16.1 cost-bracket rule remains superseded exactly as in v7.2 and v7.3.

> **Candidate status:** **FRAMEWORK V7.4 ZERO-WINTER-ROLE / PV-ROUTING / \(\kappa\)-CLAIM-BOUNDARY SUCCESSOR — CANDIDATE**. This document is **not** `CLOSED`, **not** `ACCEPTED`, **not** `FINAL`, and **not** `PROMOTED`. **An independent read-only Framework audit is required before promotion to methodology authority.** Dependent governance state at candidate creation: Registry v7.4 = `NOT_YET_CREATED / NOT_YET_AUTHORIZED`; Layer A robustness/preregistration checkpoint = `NOT_YET_CREATED / NOT_YET_AUTHORIZED`; production-authority re-freeze under v7.4 = `NOT_YET_AUTHORIZED`; Full81 under v7.4 = `NOT AUTHORIZED`. This authoring pass performed **zero** model constructions, **zero** `optimize()` calls, **zero** MILP solves, **zero** EOB or core-three reruns, and **zero** sensitivity solves, and changed no code, data, model, or parameter artifact.
...
### 下一個 governance / production-interface gates（v7.4 candidate 視角）

本小節為 **governance sequencing**，不是 implementation-status 宣告。**live implementation / production status 一律以 `docs/ai_handoff/CURRENT_STATE.md` 與 live repository evidence 為準**，不得由本框架文字推定；v7.3-era 的 downstream status 陳述在本文件中僅作 inherited 歷史語境。在任何 **v7.4** 方法權威被引用於 production 之前，必須依序完成：

- **independent read-only Framework audit 接受本 v7.4 candidate**（candidate 不得 self-accept）；
- 建立 Registry v7.4 successor 並獨立審核接受，使 framework–registry pair 同版對齊；在 Framework 被接受前不得開始 Registry successor 工作；
- 建立 Layer A robustness/preregistration checkpoint 並獨立審核；
- 完成 final cross-document alignment audit；
- 依需要重新 freeze production authority；
- 之後才是 Full81。
- 本 documentation pass 不建立、不改名、不授權任何 canonical artifact，也不判定任何既有 EOB / core-three / production 結果的續存效力（見 §36.6）；
...
9. winter-PV role：17a long-block/monthly holdout、17b sensitivity-only、corrected Sep-18 run 與 17c paired comparison 均保留為歷史/candidate evidence；planning layer 以 accepted reconstructed PV 為 mainline，zero-winter 保留為 **historical conservative validation / provenance evidence**，**不是**必跑的 downstream sensitivity（見 §36.1）。
...
本 authoring pass 只改變 methodology / document role definitions。

- 本 pass **未授權** 任何 EOB rerun、core-three rerun、Full81 run、zero-winter solve 或其他 sensitivity solve；
- 本 pass 執行的 optimization calls = **0**；model constructions = **0**；MILP solves = **0**；
- 既有 EOB / core-three 結果的**續存效力，必須由所要求的獨立 Framework-successor
  downstream-impact audit 明確裁定**；本文件**不**作此自我認定；
- 本文件不宣稱 "EOB definitely remains valid"，亦不宣稱 "core-three definitely remains valid"；
- Framework 版本改變本身**不**自動構成 rerun 依據；
- live implementation / production status 一律以 `docs/ai_handoff/CURRENT_STATE.md` 與 live repository
  evidence 為準，不得由本框架文字推定。

## 36.7 Current authorization state at candidate creation

| Layer | State |
|---|---|
| Change A / B / C methodology-role decisions | **PROPOSED IN THIS CANDIDATE**（pending independent audit） |
| Framework v7.4 | **CANDIDATE**；independent audit required；not accepted |
...
| Production-authority re-freeze under v7.4 | `NOT_YET_AUTHORIZED` |
| Full81 under v7.4 | `NOT AUTHORIZED` |
| EOB / core-three continuing validity under v7.4 | `NOT_YET_ADJUDICATED`（獨立 downstream-impact audit 裁定） |
...
> **FRAMEWORK V7.4 ZERO-WINTER-ROLE / PV-ROUTING / \(\kappa\)-CLAIM-BOUNDARY SUCCESSOR — CANDIDATE**
...
> **Framework v7.4 is NOT the accepted methodology authority. A fresh independent read-only Framework
> audit is required before any Registry successor work may begin.**
